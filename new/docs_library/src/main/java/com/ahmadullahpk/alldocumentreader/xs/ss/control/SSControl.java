...
//                getMainFrame().getActivity().setTitle((String)obj);
            ...
            case EventConstant.APP_ZOOM_ID:
                ...
                int[] params = (int[]) obj;
                float zoomValue = params[0] / (float) MainConstant.STANDARD_RATE;
                Log.d("tag", "actionEvent:"+zoomValue);
                spreadSheet.setZoom(zoomValue); //zoom
                spreadSheet.post(new Runnable() {
                    @Override
                    public void run() {
                        if (!isDispose) {
                            getMainFrame().changeZoom(zoomValue);
                            updateStatus();
                        }
                    }
                });
...
//    }
    ...
    public SysKit getSysKit() {
        ...
        else if(mainControl.getSysKit() == null){
            return null;
        }
