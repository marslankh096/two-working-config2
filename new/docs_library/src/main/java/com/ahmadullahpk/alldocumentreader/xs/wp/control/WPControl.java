    ...
    public void actionEvent(int actionID, final Object obj) {
        switch (actionID) {
            ...
            case EventConstant.APP_ZOOM_ID:
                ////////////////////////////////
                int objectValues[] = (int[]) obj;
                float value = (objectValues[0] / (float) MainConstant.STANDARD_RATE);
                if(value ==1f){
                    bigZoomValues = value;
                    wpView.setZoom(value, objectValues[1], objectValues[2]);
                }
                else if(value == 3f){
                    bigZoomValues = value;
                    wpView.setZoom(value, objectValues[1], objectValues[2]);
                }
                else if(value > bigZoomValues){
                    bigZoomValues = value * 1.1f;
                    if(bigZoomValues > 3){
                        wpView.setZoom(value * 1.1f, objectValues[1], objectValues[2]);
                    }
                    else{
                        wpView.setZoom(value, objectValues[1], objectValues[2]);
                        bigZoomValues = value;
                    ...
                    wpView.setZoom(value, objectValues[1], objectValues[2]);
                    bigZoomValues = value;
                    if (bigZoomValues > wpView.originalZoom) {
                        wpView.setZoom(value / 1.1f, objectValues[1], objectValues[2]);
                    } else {
                        wpView.setZoom(value, objectValues[1], objectValues[2]);
                        bigZoomValues = value;
                ...
                wpView.post(new Runnable() {
                    ...
                    public void run() {
                        if (!isDispose) {
                            getMainFrame().changeZoom(wpView.getZoom());
                        }
                    }
                });
                break;
...
//                break;
            ...
            case EventConstant.WP_LAYOUT_COMPLETED:
                if (wpView != null) {
                    ...
                    if (wpView.getParent() == null) {
                        if(mainControl !=null){getMainFrame().completeLayout();}
                        break;
                    }
                    wpView.post(new Runnable() {
                        @Override
