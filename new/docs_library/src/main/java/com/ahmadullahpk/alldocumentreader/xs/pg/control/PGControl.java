...
//                    pgView.setZoom(params[0] / (float) MainConstant.STANDARD_RATE, params[1], params[2]);
                    float ppZoomValue = params[0] / (float) MainConstant.STANDARD_RATE;
                    pgView.setZoom(ppZoomValue, params[1], params[2]);
                    pgView.post(new Runnable() {
                        @Override
                        public void run() {
                            if (!isDispose) {
                                getMainFrame().changeZoom(ppZoomValue);
                            }
                        }
                    });
                }
...
//                });
//                break;
            case EventConstant.PG_SLIDESHOW_DURATION:// crash location
                try{pgView.setAnimationDuration((Integer) obj);}
...
//            safeAsyncTask.execute((Void)null);
     ...
     *
     */
    public IMainFrame getMainFrame() {// crash location
    ...
    public IFind getFind() {
        if(pgView != null){return pgView.getFind();} else{return null;}
    }
