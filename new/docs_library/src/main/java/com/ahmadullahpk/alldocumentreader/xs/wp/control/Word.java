...
//            printWord.init();
    ...
    public void onDraw(Canvas canvas) {
        try{
            float zoomValue = this.zoom;
            if(zoomValue == 0f){
                this.zoom = normalZoom = getFitZoom();
                originalZoom = getFitZoom();
            }
        }
        catch (Exception e){
...
//                        printWord.setBackgroundDrawable((Drawable) bg);
                        printWord.setBackground((Drawable) bg);
                    }
                }
    ...
    private void drawPageNubmer(Canvas canvas, float zoom) {
        ...
        if (control.getMainFrame().isDrawPageNumber() && pageRoot != null) {
            ...
            int y = (int) (paint.descent() + 30);

//            Drawable drawable = SysKit.getPageNubmerDrawable();
//            drawable.setBounds((int) (x - 10), y - 10, x + w + 10, y + h + 10);
            Drawable drawable = SysKit.getPageNubmerDrawable();
            drawable.setBounds((int) (x - 65), y - 15, x + w + 50, y + h + 15);
            drawable.draw(canvas);

//            y -= paint.ascent();
            canvas.drawText(pn, x - 25, y + 25, paint);
        }
...
//     */
    ...
    /**
     * page to image for page number (base 0)
     *
    ...
    public Bitmap pageToImage(int pageNumber) {
        ...
        if (view == null) {
            ...
            view.setSize(view.getWidth() * 2, view.getHeight() * 2);
        }

        Bitmap bitmap = Bitmap.createBitmap(view.getWidth(), view.getHeight(), Config.ARGB_8888);
        Canvas canvas = new Canvas(bitmap);
        canvas.translate(-view.getX()*2, -view.getY()*2);
        canvas.drawColor(Color.WHITE);
        ((PageView) view).draw(canvas, 0, 0, 2f);
        return bitmap;
    }
...
//            printWord.setBackgroundDrawable(d);
    ...
    //
    protected float zoom = 0f;//1.f
    //
    private float normalZoom = 0f;//1.f
    //
    protected float originalZoom = 0f;
