...
//     */
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        try {
            ...
            else if(control.getSysKit() == null){

            }
            else if(control.getSysKit().getErrorKit() !=null){

            }
            else{
                if(control !=null){
                    if(control.getSysKit() != null){
                        if(control.getSysKit().getErrorKit() !=null){
    ...
    protected boolean zoom(MotionEvent event) {
        ...
        float latestZoomFit =0.5f;
        float calculatedZoomValue = ((Float) control.getActionValue(EventConstant.APP_ZOOM_ID, null)).floatValue();
        boolean isMinZoom = (int) (calculatedZoomValue * MainConstant.STANDARD_RATE) == (int) (latestZoomFit * MainConstant.STANDARD_RATE);
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(distance - dist) > 8) {
                    ...
                    if (Math.abs(calculatedZoomValue - latestZoomFit) < 1 && !increased && isMinZoom) { // 0.01
                        //current zoom rate is ZOOM_MIN_RATE, can not decrease it
                        zoomRateChanged = false;
                    } else if (Math.abs(calculatedZoomValue - 5.0f) < 0.001 && increased) { //3.0f
                        //current zoom rate is ZOOM_MAX_RATE, can not increase it
                        zoomRateChanged = false;
                    } else {
                        calculatedZoomValue = increased ? calculatedZoomValue + 0.2f : calculatedZoomValue - 0.2f; //0.1f
                        //zoomRate ranges from ZOOM_MIN_RATE to ZOOM_MAX_RATE
                        if (calculatedZoomValue > 5.0f) {//3.0f
                            calculatedZoomValue = 5.0f;//3.0f
                        } else if (calculatedZoomValue < latestZoomFit)//MainConstant.ZOOM_MIN_RATE)
                        {
                            calculatedZoomValue = latestZoomFit;//MainConstant.ZOOM_MIN_RATE;
                        }
                        // 当前zoom为最少zoom，为了zoom必须是50%、60%、70%、80%、90%、100%、110%、120%、130%、140%等
                        // 故需要做一下修正
                        if (increased && isMinZoom) {
                            calculatedZoomValue = ((int) (calculatedZoomValue * 10)) / 10.f;
                        }
                        zoomRateChanged = true;
                    }
        ...
        if (zoomRateChanged) {
            isScroll = true;
            zoomChange = true;
            control.actionEvent(EventConstant.APP_ZOOM_ID, new int[]{(int) (calculatedZoomValue * MainConstant.STANDARD_RATE), midXDoublePoint, midYDoublePoint});
