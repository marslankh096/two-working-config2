        ...
        if (shape instanceof GroupShape) {
            ...
            if (shape.getType() == AbstractShape.SHAPE_SMARTART) {
                ...
                // table
                    ...
                    drawTables(canvas, pgModel, editor, slideNo, (TableShape) shape, zoom, shapeVisible);
                }
            }
        }
...
//     */
    private void drawTables(
            ...
            IAnimation>> pptShapeVisibility
    ) {
        int pptAlphaValue = 255;

        pptCanvas.save();

        processRotation(pptCanvas, pptTable, pptZoomValue);

        if (pptTable.getAnimation() != null ) {

            if((pptTable.getAnimation().getCurrentAnimationInfor().getAlpha()) != 255){

                pptAlphaValue = pptTable.getAnimation().getCurrentAnimationInfor().getAlpha();

                if (pptTable.getBounds() != null) {

                    pptCanvas.saveLayerAlpha(
                            (pptTable.getBounds().x * pptZoomValue),
                            (pptTable.getBounds().y * pptZoomValue),
                            (pptTable.getBounds().x + pptTable.getBounds().width + 1) * pptZoomValue,
                            (pptTable.getBounds().height + pptTable.getBounds().y + 1) * pptZoomValue,
                            pptTable.getAnimation().getCurrentAnimationInfor().getAlpha(),
        ...
        int counterValue = 0;
        while (counterValue < pptTable.getCellCount()) {

            if (pptTable.getCell(counterValue) != null) {

                brRect.set(
                        Math.round(
                        pptTable.getCell(counterValue).getBounds().getX() * pptZoomValue),
                        Math.round(pptTable.getCell(counterValue).getBounds().getY() * pptZoomValue),
                        Math.round((pptTable.getCell(counterValue).getBounds().getX() + pptTable.getCell(counterValue).getBounds().getWidth()) * pptZoomValue),
                        Math.round((pptTable.getCell(counterValue).getBounds().getY() + pptTable.getCell(counterValue).getBounds().getHeight()) * pptZoomValue)
                );
                ...
                BackgroundDrawer.drawBackground(
                        ...
                        editor.getControl(),
                        pptSlideNo,
                        pptTable.getCell(counterValue).getBackgroundAndFill(),
                        brRect,
                        null,
                        pptZoomValue);
                ...
                drawCellBorder(
                        pptCanvas,
                        pptTable.getCell(counterValue),
                        pptTable.getCell(counterValue).getBounds(),
                        pptZoomValue
                );

                if (pptTable.getCell(counterValue).getText() != null) {
                    drawTextShape(
                            pptCanvas,
                            pgModel,
                            editor,
                            pptSlideNo,
                            pptTable.getCell(counterValue).getText(),
                            pptZoomValue,
                            pptShapeVisibility
                    );
                }
