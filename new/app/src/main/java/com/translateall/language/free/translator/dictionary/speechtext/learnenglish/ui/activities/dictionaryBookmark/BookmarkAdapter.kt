...
class BookmarkAdapter(
    private val callbacks: BookmarkWordCallbacks
) : RecyclerView.Adapter<BookmarkAdapter.ViewHolder>() {

    private val bookmarkList: ArrayList<BookmarkWord> = ArrayList()
    ...
    fun updateList(list: List<BookmarkWord>) {
        bookmarkList.clear()
        bookmarkList.addAll(list)
        notifyDataSetChanged()
    }

    val data: List<BookmarkWord>
        get() = bookmarkList
    ...
    fun setChecked(position: Int) {
        ...
        if (!isAnyItemSelected && !selection) {
            ...
            if (allSelectedItems.size >= bookmarkList.size) {
    ...
    interface BookmarkWordCallbacks {
        fun onWordCalled(word: String, position: Int)
        fun onCopyCalled(item: BookmarkWord)
        fun onBookmarkCalled(item: BookmarkWord)
        fun onShareCalled(item: BookmarkWord)
